...
namespace ProjectExample.Models.ModelView
{
    public class ScheduleView
    {
        ...
        public void InsertSchedule(int? id1, int? id2, int? id3, string code_ca, DateTime dateTimeStart, DateTime dateTimeEnd, int status)
        {
            try
            {
                if (code_ca != null && dateTimeStart != null && dateTimeEnd != null)
                {
                    repository.AddInfo(id1,id2,id3,code_ca,dateTimeStart,dateTimeEnd,status);
                }
                else
                {
